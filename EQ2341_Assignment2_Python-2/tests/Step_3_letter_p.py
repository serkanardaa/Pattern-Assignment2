...
##########################
# STEP 3- CASE FOR SHOWING FEATURES OF SIMILAR SYMBOLS WITH DIFFERENT POSITIONS AND SCALES

#Get symbol-1 (small p on top left quadrant)
dc1 = np.load("P_top_left.npy")

#Get symbol-2 (small p on bottom right quadrant)
dc2 = np.load("P_bottom_right.npy")

#Get symbol-3(twice as big p filling entire window)
dc3 = np.load("P_big.npy")
